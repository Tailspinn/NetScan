...
>NetworkScan.py -s 192.168.1.1-192.168.1.10 -p 443,1433      #Scan a range, for ports 443 and 1433
>Networkscan.py -m -s 192.168.1.1/24 -t 50    #Scan an entire network with 50 threads, suppresses extra output """)
    parser.add_argument('-b','--bind', type=str,metavar='<IP>' , help="Specify a local IP interface to bind.")
    parser.add_argument('-p','--ports', type=str,metavar='<ports>' , help="Specify ports to scan default is -p \"22,80,81,443,3389,8080\"")
    parser.add_argument("-m","--module",action="store_true",help="Run as a module, supresses extra output, returns a list object.")
    parser.add_argument('-t','--threads', type=int,metavar='##', help="Specify number of threads. Default is 30")
    parser.add_argument('-s','--scantarget', type=str,metavar='<IP>' , help="Specify an IP, mutiple IPs, range or network to scan examples below")
